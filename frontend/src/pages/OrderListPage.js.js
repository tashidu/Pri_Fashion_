...
const OrderListPage = () => {
  ...
  const handleSubmitOrder = async (orderId) => {
    const confirm = window.confirm("Are you sure you want to submit this order?");
    if (!confirm) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            fetchOrders(); // If you have a function to refresh order list
        } else {
            ...
            <table className="w-full border border-gray-300">
              ...
              <tbody>
                {selectedOrderItems.map((item, index) => (
                  <tr key={index}>
                    <td className="border px-4 py-2">
                      {item.finished_product_name || item.finished_product}
                    </td>
                    <td className="border px-4 py-2">
